...
<head>
    ...
    <style>
        ...
        .pagina-header .subtitulo { color: #888; font-size: 13px; margin-top: 16px; }
...
<div class="main-content">
    ...
    <div class="secao">
        ...
        <?php if ($refPortfolio): ?>
            <?php
                // A imagem é um NOME DE FICHEIRO (produto_imagem.imagem); o ficheiro
                // está em public/imagens/produtos/. Só a mostramos se existir mesmo.
                $refFicheiro = $refPortfolio['imagem'] ?? '';
                $refSrc = '';
                if (!empty($refFicheiro) && file_exists(__DIR__ . '/../../imagens/produtos/' . $refFicheiro)) {
                    $refSrc = '../../imagens/produtos/' . rawurlencode($refFicheiro);
                }
            ?>
            <div style="background:#fff8fa; border:1px solid #f4cdd5; border-radius:12px; padding:16px; margin-bottom:18px; display:flex; gap:16px; align-items:center; flex-wrap:wrap;">
                ...
                <?php else: ?>
                    <div style="width:90px; height:90px; border-radius:10px; border:1px solid #f0e3e7; background:#fff; display:flex; align-items:center; justify-content:center; flex-shrink:0;">
